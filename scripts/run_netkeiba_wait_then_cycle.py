...
from racing_ml.common.artifacts import absolutize_path, display_path as artifact_display_path, ensure_output_file_path as artifact_ensure_output_file_path, read_json, utc_now_iso, write_json
...
def log_progress(message: str) -> None:
    ...
    print(f"[netkeiba-wait-cycle {now}] {message}", flush=True)
...
def _safe_write_wait_manifest(path: Path, payload: dict[str, Any]) -> None:
...
def _write_wait_manifest(path: Path, payload: dict[str, Any]) -> None:
    _safe_write_wait_manifest(path, payload)
...
def main() -> int:
    ...
    payload: dict[str, Any] = {
        ...
        "paths": {
            "lock_file": None,
            "crawl_manifest": None,
            "active_backfill_manifest": artifact_display_path(active_backfill_manifest_path, workspace_root=ROOT),
            "active_pedigree_manifest": artifact_display_path(active_pedigree_manifest_path, workspace_root=ROOT),
    ...
    try:
        ...
        artifact_ensure_output_file_path(gate_manifest_path, label="gate manifest output", workspace_root=ROOT)
        ...
        payload["paths"] = {
            "lock_file": artifact_display_path(lock_path, workspace_root=ROOT),
            "crawl_manifest": artifact_display_path(crawl_manifest_path, workspace_root=ROOT),
            "active_backfill_manifest": artifact_display_path(active_backfill_manifest_path, workspace_root=ROOT),
            "active_pedigree_manifest": artifact_display_path(active_pedigree_manifest_path, workspace_root=ROOT),
            "backfill_manifest": artifact_display_path(backfill_manifest_path, workspace_root=ROOT),
            "snapshot_output": artifact_display_path(snapshot_path, workspace_root=ROOT),
        ...
        progress.start(message="wait manifest initialized")
        ...
        with Heartbeat("[netkeiba-wait-cycle]", "running handoff backfill cycle", logger=log_progress):
