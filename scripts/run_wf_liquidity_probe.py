...
def log_progress(message: str) -> None:
    ...
    print(f"[wf-liquidity-probe {now}] {message}", flush=True)
...
def main() -> int:
    ...
    try:
        ...
        progress = ProgressBar(total=6, prefix="[wf-liquidity-probe]", logger=log_progress, min_interval_sec=0.0)
        progress.start(message="configs loaded")

        _log("loading training table")
        with Heartbeat("[wf-liquidity-probe]", "loading training table", logger=log_progress):
        ...
        with Heartbeat("[wf-liquidity-probe]", "building features", logger=log_progress):
        ...
        with Heartbeat("[wf-liquidity-probe]", f"loading model {model_path.name}", logger=log_progress):
        ...
        with Heartbeat("[wf-liquidity-probe]", "running inference", logger=log_progress):
        ...
        progress.update(message=f"inference complete rows={len(pred):,}")
        ...
        progress.update(message=f"wf slices ready total={len(nested_slices)} selected={len(selected_folds)}")

        report_rows: list[dict[str, object]] = []
        fold_summaries: list[dict[str, object]] = []
        fold_progress = ProgressBar(total=max(len(selected_folds), 1), prefix="[wf-liquidity folds]", logger=log_progress, min_interval_sec=0.0)
        fold_progress.start(message="analyzing selected folds")
        for fold_index, (train_df, valid_df, test_df) in enumerate(nested_slices, start=1):
            ...
            fold_progress.update(message=f"fold={fold_index} candidates={len(fold_rows)}")
        ...
        with Heartbeat("[wf-liquidity-probe]", "writing probe outputs", logger=log_progress):
            write_text_file(summary_path, json.dumps(payload, ensure_ascii=False, indent=2), label="summary output")
            ...
            if not detail_frame.empty:
                ...
                write_csv_file(detail_path, pd.DataFrame(), index=False, label="detail output")
        ...
        progress.complete(message="liquidity probe completed")
        return 0
    except KeyboardInterrupt:
        print("[wf-liquidity-probe] interrupted by user")
        return 130
    except (ValueError, FileNotFoundError, IsADirectoryError, RuntimeError) as error:
        print(f"[wf-liquidity-probe] failed: {error}")
        return 1
