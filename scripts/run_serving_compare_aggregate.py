...
def main() -> int:
    ...
    parser.add_argument("--dashboard-summaries", nargs="+", required=True)
    ...
    try:
        summary_paths = [resolve_path(path) for path in args.dashboard_summaries]
        output_summary = resolve_path(args.output_summary)
        output_chart = resolve_path(args.output_chart)
        output_csv = resolve_path(args.output_csv)

        progress.start(message=f"loading dashboard summaries count={len(summary_paths)}")
        ...
        progress.update(message="dashboard summaries loaded")
        ...
        for path, payload in zip(summary_paths, payloads):
            ...
            row = {
                ...
                "summary_file": display_path(path),
        ...
        aggregate_payload = {
            "dashboard_summaries": [display_path(path) for path in summary_paths],
