  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .tab.active {
      ...
      background: var(--navy);
      color: white;
      box-shadow: 0 10px 22px rgba(33, 57, 91, 0.22);
    }
    .chip {
      display: inline-flex;
      align-items: center;
...
<body>
  ...
  <script>
    ...
    const focusColumnMeta = {
      ...
      pred_rank: { label: "命順", description: "score ベースの順位。", className: "tiny-col" },
      ev_rank: { label: "EV順", description: "raw EV ベースの順位。", className: "tiny-col" },
      ...
      score: { label: "score", description: "命中寄り評価。高いほど能力評価が高い。", className: "num-col" },
      expected_value: { label: "生EV", description: "score × odds の生期待値。", className: "num-col" },
      policy_prob: { label: "P確率", description: "runtime policy が使う blended probability。", className: "num-col" },
      policy_expected_value: { label: "PEV", description: "policy 用期待値。1.0 超でプラス期待値読み。", className: "num-col" },
    ...
    function shortText(value, maxLength = 8) {
    ...
    function decorateRace(race) {
      ...
      race.dismissSummary = dismissed.length
        ...
        : "消印なし";
      race.raceCommentary = [
        race.selectedRows > 0 ? `runtime policy では ${race.selectedRows} 頭を実選択。` : "runtime policy では見送り。",
        marked.length ? `総合期待値ベースの印は ${race.recommendationSummary}。` : "強い印を打つほどの集中候補は薄め。",
        race.topPolicyEvHorse ? `${race.topPolicyEvHorse} が policy EV の先頭。` : "",
        race.blocker ? `全体 blocker は ${race.blocker}。` : "",
      ].filter(Boolean).join(" ");
    ...
    function renderVenueTabs() {
      const buttons = [
        ...
        ...state.data.venues.map((venue) => {
          ...
          return `<button class="tab ${activeClass}" data-venue-code="${venue.venueCode || ""}">${venue.venue || "Unknown"}</button>`;
        }),
      ];
