...
def log_progress(message: str) -> None:
    ...
    print(f"[wf-family-compare {now}] {message}", flush=True)
...
def main() -> int:
    ...
    try:
        ...
        progress = ProgressBar(total=3, prefix="[wf-family-compare]", logger=log_progress, min_interval_sec=0.0)
        ...
        with Heartbeat("[wf-family-compare]", "building family comparison", logger=log_progress):
            report, summary_df = _build_family_compare(compare_payload, signature)
        progress.update(message=f"family compare built variants={report.get('family_variant_count')}")
        with Heartbeat("[wf-family-compare]", "writing family comparison outputs", logger=log_progress):
        ...
        progress.complete(message="family comparison completed")
        return 0
    except KeyboardInterrupt:
        print("[wf-family-compare] interrupted by user")
        return 130
    except (ValueError, FileNotFoundError, IsADirectoryError, RuntimeError) as error:
        print(f"[wf-family-compare] failed: {error}")
        return 1
