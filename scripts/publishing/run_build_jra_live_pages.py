...
<body>
  ...
  <script>
    ...
    function buildLiveHarvillePayload(race, analyzeData) {
      ...
      return {
        available: marketOptions.length > 0,
        message: "最新オッズを再取得し、model score 正規化勝率と単勝市場の暗黙確率をブレンドして Harville 理論オッズを再計算しました。",
        meta: {
    ...
    async function refreshHarvilleOdds(race) {
      ...
      try {
        ...
        race.liveHarville = buildLiveHarvillePayload(race, analyzeData);
