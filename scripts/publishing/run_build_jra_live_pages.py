...
<body>
    ...
    <section class="section race-layout race-section" id="race-data-section" hidden>
      <div>
        ...
        <section class="market-card race-panel" id="harville-panel" hidden>
          ...
          <div class="harville-anchor-card">
  ...
  <script>
    ...
    function buildHarvilleOverviewMarkets(rowsByMarket, winCompareRows) {
      const markets = [...availableHarvilleMarkets(rowsByMarket || {})];
      if (!Array.isArray(winCompareRows) || !winCompareRows.length) {
        return markets;
      }
      const winCompareMarket = { key: "win_compare", label: "単勝" };
      const quinellaIndex = markets.findIndex((market) => market.key === "quinella");
      if (quinellaIndex >= 0) {
        markets.splice(quinellaIndex, 0, winCompareMarket);
        return markets;
      }
    ...
    function harmonicMean(values) {
      ...
      const denominator = numeric.reduce((total, item) => total + (1 / item), 0);
      if (!(denominator > 0)) {
        return null;
      }
    ...
    async function fetchRaceAnalyzePayload(raceId, { forceRefresh = false } = {}) {
      const cacheKey = `${raceId}:${forceRefresh ? "refresh" : "cached"}`;
      if (!forceRefresh && oddsAnalyzeCache.has(cacheKey)) {
        return oddsAnalyzeCache.get(cacheKey);
      }
      if (oddsAnalyzePending.has(cacheKey)) {
        return oddsAnalyzePending.get(cacheKey);
      }
      const request = fetch(`${ODDS_API_BASE_URL}/v1/analyze`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          race_url: buildRaceUrl({ raceId }),
          excluded_horses: [],
        ...
        oddsAnalyzeCache.set(cacheKey, payload);
        return payload;
      }).finally(() => {
        oddsAnalyzePending.delete(cacheKey);
      });
      oddsAnalyzePending.set(cacheKey, request);
      return request;
    }

    function buildLiveHarvillePayload(race, analyzeData) {
      const comparisons = buildHarvilleComparisons(race, analyzeData);
      const marketOptions = availableHarvilleMarkets(comparisons.rowsByMarket).map((item) => ({
        key: item.key,
        label: item.label,
      ...
      return {
        available,
        message: available
          ? "最新オッズを再取得し、model score 正規化勝率と単勝市場の暗黙確率をブレンドして Harville 理論オッズを再計算しました。"
          : "latest odds were fetched, but no Harville comparisons could be rebuilt. build snapshot is still available.",
        meta: {
          oddsUpdatedAt: raceMeta.odds_updated_at || null,
          analyzedAt: raceMeta.analyzed_at || null,
          horseCount: comparisons.masterCount,
          positiveRows: Array.isArray(comparisons.screenerRows) ? comparisons.screenerRows.length : 0,
          probabilitySource: "blend:model35_market65",
    ...
    function activeHarvillePayload(race) {
      if (!race) {
        return null;
      }
      return race.liveHarville || race.harville || null;
    }

    function normalizeOddsRefreshError(error) {
      const message = error instanceof Error ? error.message : String(error || "unknown error");
      if (/Failed to fetch|Load failed|NetworkError/i.test(message)) {
        return "browser blocked direct odds refresh (likely CORS). build snapshot is still available.";
      }
      if (/HTTP\s+4\d\d/i.test(message)) {
        return `odds api rejected refresh request: ${message}`;
      }
      return `odds api unavailable: ${message}`;
    }
    ...
    function buildRaceHorseMaster(race, analyzeData) {
      ...
      for (const row of analyzeEntries) {
        const horseNo = normalizeHorseNo(row?.["馬番"] || row?.col_2 || row?.col_1);
        const horseName = normalizeHorseName(row?.["馬名"] || row?.col_4);
      ...
      for (const row of race.rowsData || []) {
        ...
        const horseNo = normalizeHorseNo(row?.horse_no || row?.gate_no || row?.horseNumber || horseIdSuffix);
        const horseName = normalizeHorseName(row?.horse_name);
        if (!horseNo || seen.has(horseNo)) {
          continue;
        }
    ...
    function formatHarvilleHorseOptionLabel(item) {
      if (!item) {
        return "";
      }
      const horseNo = normalizeHorseNo(item.horseNo);
      const horseName = normalizeHorseName(item.horseName);
      return [horseNo, horseName].filter(Boolean).join(" ") || horseNo || horseName || "";
    }

    function buildHarvilleHorseNameMap(race, marketKey) {
      const horseNameMap = new Map();
      for (const option of getHarvilleExcludeOptions(race)) {
        if (option.horseNo && option.horseName && !horseNameMap.has(option.horseNo)) {
          horseNameMap.set(option.horseNo, option.horseName);
        }
      }
      for (const option of getHarvilleAnchorOptions(race, marketKey, 0)) {
        if (option.horseNo && option.horseName && !horseNameMap.has(option.horseNo)) {
          horseNameMap.set(option.horseNo, option.horseName);
        }
      }
      for (const option of getHarvilleAnchorOptions(race, marketKey, 1)) {
        if (option.horseNo && option.horseName && !horseNameMap.has(option.horseNo)) {
          horseNameMap.set(option.horseNo, option.horseName);
        }
      }
    ...
    function renderHarvilleAnchorControls(race, anchors, marketKey) {
      ...
      const horseNameMap = buildHarvilleHorseNameMap(race, marketKey);
      const selectedAnchorLabels = selectedAnchors
        .map((horseNo) => formatHarvilleHorseOptionLabel({ horseNo, horseName: horseNameMap.get(horseNo) }))
        .filter(Boolean);
      const anchorMeta = marketKey === "win_compare"
        ...
        : (selectedAnchorLabels.length
          ? `${selectedAnchorLabels.join(" / ")} を表示中。未指定に戻すと全表示です。`
      ...
      document.getElementById("harville-anchor-selects").innerHTML = marketKey === "win_compare" ? "" : Array.from({ length: maxAnchors }, (_, index) => {
        ...
        const optionHtml = [
          ...
          ...options.map((item) => {
            ...
            return `<option value="${escapeHtml(item.horseNo)}"${selected}${disabled}>${escapeHtml(formatHarvilleHorseOptionLabel(item))}</option>`;
          }),
        ].join("");
    ...
    function harvilleSummaryTableHtml(rows, anchors, sortState, exhaustive = false) {
      ...
      const visibleRows = exhaustive ? sortHarvilleRows(rows, sortState, anchors) : sortHarvilleRows(rows, sortState, anchors).slice(0, 16);
      ...
      return `<table class="compact-table"><thead><tr>${[
        harvilleHeaderCell("券種", "marketLabel", sortState, "券種で並べ替え"),
        harvilleHeaderCell("対象", "target", sortState, "組み合わせで並べ替え"),
        harvilleHeaderCell("実オッズ", "market_odds", sortState, "市場オッズで並べ替え"),
        harvilleHeaderCell("Harville", "harville_odds", sortState, "Harville 理論オッズで並べ替え"),
        harvilleHeaderCell("EV倍率", "ev_ratio", sortState, "EV 倍率で並べ替え"),
        harvilleHeaderCell("上振れ", "edge", sortState, "上振れ率で並べ替え"),
      ].join("")}</tr></thead><tbody>${body}</tbody></table>`;
    }

    function collectHarvilleHorseOptions(race, rowsByMarket) {
      const seen = new Set();
      const fromRaceRows = (Array.isArray(race?.rowsData) ? race.rowsData : [])
        .map((row) => ({
          horseNo: resolveRaceRowHorseNo(row),
          horseName: normalizeHorseName(row?.horse_name),
        }));
      const fromHarvilleRows = flattenHarvilleRows(rowsByMarket || {}).flatMap((row) => [
        { horseNo: normalizeHorseNo(row?.horse_no_a), horseName: normalizeHorseName(row?.horse_name_a) },
        { horseNo: normalizeHorseNo(row?.horse_no_b), horseName: normalizeHorseName(row?.horse_name_b) },
        { horseNo: normalizeHorseNo(row?.horse_no_c), horseName: normalizeHorseName(row?.horse_name_c) },
      ]);
      return [...fromRaceRows, ...fromHarvilleRows]
        .filter((item) => item.horseNo && !seen.has(item.horseNo) && seen.add(item.horseNo))
        .sort((left, right) => horsePairSort(left.horseNo, right.horseNo));
    }
    ...
    function buildHarvilleOverviewRows(race, rowsByMarket, winCompareRows) {
      ...
      return horses.map((horse) => {
        ...
        markets.forEach((market) => {
          ...
          if (edgeRatio !== null && (bestEdge === null || edgeRatio > bestEdge)) {
            bestEdge = edgeRatio;
          }
        });
        return {
          horseNo: horse.horseNo,
          horseName: horse.horseName,
          label: `${horse.horseNo} ${horse.horseName}`.trim(),
          metrics,
          bestEdge,
        };
      });
    ...
    function harvilleOverviewTableHtml(race, rowsByMarket, winCompareRows, precomputedOverviewRows = null) {
      ...
      const headTop = `<tr><th rowspan="2">馬</th>${markets.map((market) => `<th class="harville-overview-market" colspan="2">${escapeHtml(market.label)}</th>`).join("")}</tr>`;
      const headBottom = `<tr>${markets.map(() => '<th class="harville-overview-subhead">実</th><th class="harville-overview-subhead">Harville</th>').join("")}</tr>`;
      const body = rows.map((row) => {
        const metricCells = markets.map((market) => {
      ...
      return `<table class="compact-table harville-overview-table"><thead>${headTop}${headBottom}</thead><tbody>${body}</tbody></table>`;
    }

    function harvilleDetailTableHtml(marketKey, rows, anchors, sortState) {
      if (!rows.length) {
        return '<div class="market-empty">この券種のオッズはまだ取得できていません。</div>';
      }
      const visibleRows = limitedHarvilleRows(rows, sortState, anchors, HARVILLE_DETAIL_ROW_LIMIT);
      const displayedLegCount = Math.max(...visibleRows.map((row) => harvilleDisplayLegs(row, anchors).length), 0);
      const columnLabels = getHarvilleColumnLabels(marketKey, displayedLegCount, anchors);
      const head = `<tr>${columnLabels.map((item, index) => harvilleHeaderCell(item.horse, `horse_${index}`, sortState, `${item.horse}で並べ替え`)).join("")}${columnLabels.map((item, index) => harvilleHeaderCell(item.odds, `win_odds_${index}`, sortState, `${item.odds}で並べ替え`)).join("")}${harvilleHeaderCell("実オッズ", "market_odds", sortState, "市場オッズで並べ替え")}${harvilleHeaderCell("Harville", "harville_odds", sortState, "Harville 理論オッズで並べ替え")}${harvilleHeaderCell("EV倍率", "ev_ratio", sortState, "EV 倍率で並べ替え")}${harvilleHeaderCell("上振れ", "edge", sortState, "上振れ率で並べ替え")}</tr>`;
      const body = visibleRows.map((row) => {
    ...
    function renderHarvilleSnapshot(race) {
      const button = document.getElementById("harville-refresh-button");
      const refreshError = race?.liveHarvilleStatus?.state === "error" ? race.liveHarvilleStatus.message : null;
      if (button instanceof HTMLButtonElement) {
        button.disabled = !race || state.activeOddsRaceId === race.raceId;
        button.textContent = state.activeOddsRaceId === race?.raceId ? "再取得中..." : "最新オッズで再計算";
      }
      const harville = activeHarvillePayload(race);
      ...
      const activeMarket = availableTabs.some((item) => item.key === state.harvilleMarket)
        ...
        : "overview";
      if (state.harvilleAnchorRaceId !== race?.raceId) {
      ...
      document.getElementById("harville-refresh-status").textContent = state.activeOddsRaceId === race?.raceId
        ? "updating live odds..."
        : (refreshError
          ? "live refresh blocked"
          : (harville?.meta?.snapshotSource === "live-refresh" ? "live refreshed" : "build snapshot"));
    ...
    async function refreshHarvilleOdds(race) {
      ...
      try {
        ...
        if (liveHarville.available) {
          ...
          race.liveHarvilleStatus = { state: "live", message: null };
        } else {
          race.liveHarvilleStatus = { state: "error", message: liveHarville.message };
        }
      } catch (error) {
    ...
    function compareNullableValues(leftValue, rightValue, dir) {
      ...
      return String(leftValue).localeCompare(String(rightValue), "ja") * dir;
    }
    ...
    function sortRows(rows, sortState) {
      ...
      copied.sort((left, right) => {
        ...
        return compareNullableValues(leftValue, rightValue, sortState.dir);
      });
      return copied;
    }

    function getHarvilleSortValue(row, key, anchors) {
      if (key === "target") {
        return buildHarvilleTargetLabel(row, anchors);
      }
      if (key.startsWith("horse_")) {
        const legIndex = Number(key.split("_")[1]);
        const leg = harvilleDisplayLegs(row, anchors)[legIndex];
        return leg?.horseNo || "";
      }
      if (key.startsWith("win_odds_")) {
        const legIndex = Number(key.split("_")[2]);
        const leg = harvilleDisplayLegs(row, anchors)[legIndex];
        return numericValue(leg?.winOdds);
      }
      return row?.[key] ?? null;
    }

    function sortHarvilleRows(rows, sortState, anchors) {
      const copied = [...rows];
      copied.sort((left, right) => {
        const leftValue = getHarvilleSortValue(left, sortState.key, anchors);
        const rightValue = getHarvilleSortValue(right, sortState.key, anchors);
        if (sortState.key.startsWith("horse_")) {
          const byHorse = compareNullableValues(horseSortKey(leftValue), horseSortKey(rightValue), sortState.dir);
          if (byHorse !== 0) {
            return byHorse;
          }
        }
        const byValue = compareNullableValues(leftValue, rightValue, sortState.dir);
        if (byValue !== 0) {
          return byValue;
        }
        return compareNullableValues(left.market_odds, right.market_odds, -1);
      });
      return copied;
    }

    function limitedHarvilleRows(rows, sortState, anchors, limit) {
      const sortedRows = sortHarvilleRows(rows, sortState, anchors);
      if (sortedRows.length <= limit) {
        return sortedRows;
      }
      if (sortState.key !== "edge" || sortState.dir !== -1) {
        return sortedRows.slice(0, limit);
      }
      const positiveRows = sortedRows.filter((row) => (numericValue(row.edge) ?? 0) >= 0);
      const negativeRows = sortedRows.filter((row) => (numericValue(row.edge) ?? 0) < 0);
      if (!positiveRows.length || !negativeRows.length) {
        return sortedRows.slice(0, limit);
      }
      const negativeQuota = Math.min(Math.max(Math.floor(limit / 3), 1), negativeRows.length);
      const positiveQuota = Math.min(limit - negativeQuota, positiveRows.length);
      const picked = [...positiveRows.slice(0, positiveQuota), ...negativeRows.slice(0, negativeQuota)];
      if (picked.length < limit) {
        const remainder = sortedRows.filter((row) => !picked.includes(row));
        picked.push(...remainder.slice(0, limit - picked.length));
      }
      return sortHarvilleRows(picked, sortState, anchors);
    }

    function harvilleHeaderCell(label, key, sortState, description = "") {
      const arrow = sortState.key === key ? (sortState.dir === 1 ? " ↑" : " ↓") : "";
      return `<th><button data-table="harville" data-column="${escapeHtml(key)}" title="${escapeHtml(description)}">${escapeHtml(label)}${arrow}</button></th>`;
    }
    ...
    function bindEvents() {
      document.body.addEventListener("click", (event) => {
        ...
        if (column && tableName) {
          const sortState = tableName === "focused"
            ? state.focusedSort
            : tableName === "raw"
              ? state.rawSort
              : state.harvilleSort;
