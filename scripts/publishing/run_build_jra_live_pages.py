...
<body>
  ...
  <script>
    ...
    function renderHarvilleSnapshot(race) {
      ...
      document.getElementById("harville-refresh-status").textContent = harville?.meta?.snapshotSource === "live-refresh"
        ? "live refreshed"
        : "build snapshot";
    ...
    async function refreshHarvilleOdds(race) {
      ...
      try {
        ...
        renderHarvilleError(error instanceof Error ? error.message : String(error));
      } finally {
