...
8. モデル評価（全体＋日別）
    ...
    - 複数期間・複数 profile の threshold report を並べて比較するときは `python scripts/run_wf_threshold_compare.py --reports <report1> <report2> ...` を使えます。strictest threshold (`1/3/5 folds`)、warning 数、閾値ごとの feasible fold 数 / dominant failure / support summary を JSON/CSV にまとめます
