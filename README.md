...
7. Serving smoke validation
    ...
    - runtime selector 自体の staged fallback を試す probe として [configs/model_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_june_strict_serving_staged_mitigation_probe.yaml](/workspaces/nr-learn/configs/model_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_june_strict_serving_staged_mitigation_probe.yaml) も追加しました。`serving.policy.strategy_kind=staged` に `stages` を並べると、runtime は各 race で「最初に selection が出る stage」を採用します
    - この staged selector は prediction/backtest artifact に `policy_stage_name` / `policy_stage_index` / `policy_selected_strategy_kind` を残し、backtest JSON には `policy_stage_names` も保存します。ただし現在の fallback 条件は race-local な `no_selection` のみなので、代表 5 日の staged probe はすべて `portfolio_ev_only` で止まり、shared-date total net も `30.3` と `portfolio_ev_only` 単体 probe と同じでした。fold/bankroll-stress 起因の escalation を runtime で再現するには、さらに selector 条件の拡張が必要です
    - fallback 条件を少し広げた heuristic probe として [configs/model_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_june_strict_serving_staged_mitigation_ev_guard_probe.yaml](/workspaces/nr-learn/configs/model_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_june_strict_serving_staged_mitigation_ev_guard_probe.yaml) も追加しました。これは `max_expected_value_below` で stage1/2 を飛ばす近似ですが、代表 5 日では `kelly_fallback_1` が実際に起動した一方、shared-date total net は `14.7067` に留まり baseline `39.6` を大きく下回りました。runtime hook と stage trigger 自体は動いたものの、素朴な EV guard だけでは serving 改善には足りません
    - long window の stateful selector probe では、`run_serving_smoke.py` の shared feature build が重すぎるときに `python scripts/run_serving_replay_from_predictions.py --config <variant_config> --prediction-files <既存 predictions_*.csv ...> --artifact-suffix <suffix>` で既存 prediction CSV から policy だけ差し替えた replay backtest summary を生成できます。`serving_smoke_*` 互換の summary と suffix 付き `backtest_*.json/.png` を直接出すため、heavy feature rebuild を避けたまま downstream の compare / bankroll sweep を流せます
    - `python scripts/run_serving_stateful_bankroll_sweep.py` の bankroll floor sweep は default で `1.01` を含むようにしてあります。これは「最初から variant を使う」ケースを同時に探索するためです。加えて現在は pure stage path も常に別候補として評価するので、floor grid が足りず「常に stage 2/3 を使う」path を見落とすことはありません。`2024-06-29..2024-09-15` の tail weekends 24 日で baseline `fallback_hybrid_june_strict_tail_weekends` と `portfolio_ev_only_hybrid_keep_kelly` / `portfolio_lower_blend_hybrid_keep_kelly` を replay 比較したところ、lower-blend の pure path は baseline-only final bankroll `0.6922` に対し `0.8646`、EV-only の pure path は `0.9569` でした。主因は portfolio month の overbet 抑制で、特に `2024-08-03` の損失が大きく縮みます
    - 同じ replay 手法を late-September 5 日 (`2024-09-16/21/22/28/29`) に適用しても方向は変わりませんでした。baseline `current_recommended_serving_late_sep` は `31` bets / total net `-25.6` / final bankroll `0.2780` まで落ちた一方、`portfolio_lower_blend_hybrid_keep_kelly_late_sep` は `1` bet / total net `-1.0` / final bankroll `0.9583` で、stateful sweep でも best path は再び `floors=[1.01, 1.01]` の lower-blend 単独でした。特に `2024-09-16` は baseline が `11` bets で daily multiplier `0.5417` だったのに対し、lower-blend は no-bet で `1.0` を維持しました
    - この conservative candidate は stable alias `current_bankroll_candidate` でも引けます。実体は [configs/model_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_june_strict_serving_portfolio_lower_blend_hybrid_keep_kelly.yaml](/workspaces/nr-learn/configs/model_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_june_strict_serving_portfolio_lower_blend_hybrid_keep_kelly.yaml) で、May-July の Kelly override を残したまま Aug+ default portfolio の `blend_weight` を `0.6` へ落とした検証用 profile です
    - EV floor 強化だけを見たい比較用 alias として `current_ev_candidate` も追加しました。実体は [configs/model_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_june_strict_serving_portfolio_ev_only_hybrid_keep_kelly.yaml](/workspaces/nr-learn/configs/model_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_june_strict_serving_portfolio_ev_only_hybrid_keep_kelly.yaml) で、May-July の Kelly override を残したまま Aug+ default portfolio の `min_expected_value` を `1.0` に上げた検証用 profile です
    - 実 window の挙動は candidate ごとに違います。`current_ev_candidate` は late-September では pure final bankroll `0.5832`、tail-weekends では `0.9569` まで改善しましたが、shared-date total net はそれぞれ `-12.0`, `-4.0` で、`current_bankroll_candidate` よりは攻めます。`current_bankroll_candidate` はより低ベットで pure final bankroll `0.9583` / `0.8646`、shared-date total net `-1.0` / `-10.0` なので、EV-only は intermediate candidate、lower-blend は strongest de-risk candidate という位置づけです
    - `scripts/run_serving_smoke.py` も stable model profile を直接受けるようにしたので、preset を増やさなくても `python scripts/run_serving_smoke.py --profile current_bankroll_candidate --date 2024-09-16 --date 2024-09-21 ...` のように実行できます。built-in case を持たない profile では `--date` が必須です
    - さらに `scripts/run_serving_smoke.py --prediction-backend replay-existing` を追加したので、既存の canonical `artifacts/predictions/predictions_YYYYMMDD.csv` を再利用して suffix 付き backtest と summary を生成できます。shared frame の feature build を完全にスキップするため、`current_bankroll_candidate` のような conservative variant でも heavy build なしで `serving_smoke_compare` まで通せます。late-September 5 日での検証では `current_recommended_serving` に対し `current_bankroll_candidate` が `31 -> 1` bets、total net `-25.6 -> -1.0` をそのまま再現しました
    - 同じ replay smoke を tail-weekends 24 日でも `current_bankroll_candidate` 名義で流し直し、baseline compare と bankroll sweep を stable profile ベースへ統一しました。結果は既存の lower-blend replay と一致し、baseline-only final bankroll `0.6922` に対して `current_bankroll_candidate` 単独 path は `0.8646`、shared-date bets は `82 -> 10` でした
    - `scripts/run_serving_smoke_compare.py` の既定出力名は `profile` ではなく summary の `artifact_suffix` を優先するようにしました。これで `current_bankroll_candidate` の late-sep / tail-weekends のように同一 profile を別 window で比較しても、compare artifact が上書きされません
    - stable profile 同士を replay smoke から compare までまとめて回すときは `python scripts/run_serving_profile_compare.py --left-profile current_recommended_serving --right-profile current_bankroll_candidate --date 2024-09-16 --date 2024-09-21 --date 2024-09-22 --date 2024-09-28 --date 2024-09-29 --window-label late_sep` を使えます。左右の `serving_smoke_*.json`、compare JSON/CSV、そして provenance 用 manifest をまとめて出すので、window ごとの定型比較を 1 コマンドに寄せられます
    - bankroll 観点まで同じ導線で確認したいときは上のコマンドに `--run-bankroll-sweep` を付けます。これで `serving_stateful_bankroll_sweep_<left>_vs_<right>.json/.csv` も同時に出るため、shared-date net では不利でも final bankroll で優位な conservative candidate を compare の隣で確認できます
    - 同じ runner から report まで閉じたいときは `--run-dashboard` も併用します。`run_serving_profile_compare.py --run-bankroll-sweep --run-dashboard ...` で compare manifest を基準に `artifacts/reports/dashboard/serving_compare_dashboard_<left>_vs_<right>.{json,png,csv}` を自動生成でき、比較・bankroll・可視化が 1 run で揃います
    - compare manifest から可視化つきの要約を作るときは `python scripts/run_serving_compare_dashboard.py --manifest-file artifacts/reports/serving_smoke_profile_compare_current_recommended_serving_late_sep_vs_current_bankroll_candidate_late_sep.json` を使えます。`artifacts/reports/dashboard/` に summary JSON、日次 net と bankroll path の PNG、日別 CSV を出すので、stable profile 比較の読み取りを artifact 単体で閉じられます
    - 複数 window の集約は `run_serving_compare_aggregate.py` で行えます。`--dashboard-summaries ...` に加えて、いまは `--compare-manifests ...` も受けるので、`serving_smoke_profile_compare_*.json` だけあれば window ごとの net delta / pure bankroll delta / best sweep bankroll を JSON/CSV に集約し、scatter/bar の PNG も出せます。period 横断の trade-off を profile compare 系 artifact だけで確認できます
...
8. モデル評価（全体＋日別）
    ...
    - runtime は `strategy_kind=staged` も読めるようになりましたが、現状の semantics は「race ごとに先頭 stage で bet が 1 件も出なかったら次段へ進む」です。したがって、WF diagnostics で見えた fold-level / bankroll-level stress escalation をそのまま再現するものではなく、まずは deployable な runtime hook を通した段階です
