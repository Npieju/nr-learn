...
7. Serving smoke validation
    ...
    - `python scripts/run_serving_stateful_bankroll_sweep.py` の bankroll floor sweep は default で `1.01` を含むようにしてあります。これは「最初から variant を使う」ケースを同時に探索するためです。`2024-06-29..2024-09-15` の tail weekends 24 日で baseline `fallback_hybrid_june_strict_tail_weekends` と `portfolio_ev_only_hybrid_keep_kelly` / `portfolio_lower_blend_hybrid_keep_kelly` を replay 比較したところ、best result は `floors=[1.01, 1.01]`、すなわち実質的に `portfolio_lower_blend_hybrid_keep_kelly` 単独でした。baseline-only final bankroll `0.6922` に対し best replay result は `0.8646` で、主因は `2024-08-03` の portfolio 損失が baseline の `policy_bets=4` / daily multiplier `0.8333` から lower-blend の `policy_bets=1` / daily multiplier `0.9583` に縮んだことです。EV-only branch は best path では一度も選ばれませんでした
    ...
    - 複数 window の compare dashboard summary をまとめて見たいときは `python scripts/run_serving_compare_aggregate.py --dashboard-summaries artifacts/reports/dashboard/serving_compare_dashboard_current_recommended_serving_late_sep_vs_current_bankroll_candidate_late_sep.json artifacts/reports/dashboard/serving_compare_dashboard_current_recommended_serving_tail_weekends_vs_current_bankroll_candidate_tail_weekends.json` を使えます。window ごとの net delta / pure bankroll delta / best sweep bankroll を JSON/CSV に集約し、scatter/bar の PNG も出すので、period 横断の trade-off を 1 artifact set で確認できます
