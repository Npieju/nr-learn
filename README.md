...
8. モデル評価（全体＋日別）
    ...
    - `min_bets_abs` の感度を軽く見たいときは `python scripts/run_wf_threshold_sweep.py --wf-summary artifacts/reports/wf_feasibility_diag_<config>_...json --min-bets-abs-values 100,80,60,40` を使えます。既存の WF detail CSV を再判定するだけなので、再学習や再推論なしで feasible fold 数、fold ごとの初回通過閾値、`best_feasible` / `best_fallback` の変化を JSON/CSV に保存できます
