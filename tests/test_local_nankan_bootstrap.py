...
class LocalNankanBootstrapTest(unittest.TestCase):
    ...
    def test_future_readiness_cycle_passes_cycle_scoped_bootstrap_log_prefix(self) -> None:
        ...
        self.assertEqual(wrapper_manifest["read_order"][0], "status")
        self.assertEqual(wrapper_manifest["read_order"][3], "capture_provenance.upcoming_only")
        self.assertEqual(wrapper_manifest["read_order"][6], "capture_provenance.filtered_out_count")
    ...
    def test_bootstrap_handoff_returns_not_ready_manifest(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(manifest["read_order"][3], "handoff_manifest.status")
    ...
    def test_wait_then_cycle_runs_bootstrap_followup_when_ready(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(manifest["read_order"][0], "status")
            self.assertEqual(manifest["read_order"][3], "monitor_state")
            self.assertEqual(manifest["read_order"][8], "current_refs.capture_filtered_out_count")
    ...
    def test_wait_then_cycle_updates_manifest_during_wait_interval(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(wait_payloads[0]["read_order"][0], "status")
            self.assertEqual(wait_payloads[0]["read_order"][3], "monitor_state")
            self.assertEqual(wait_payloads[0]["read_order"][8], "current_refs.capture_filtered_out_count")
            ...
            self.assertEqual(final_manifest["read_order"][0], "status")
            self.assertEqual(final_manifest["read_order"][3], "monitor_state")
            self.assertEqual(final_manifest["read_order"][8], "current_refs.capture_filtered_out_count")
            ...
            self.assertEqual(operator_board["read_order"][0], "status")
            self.assertEqual(operator_board["read_order"][3], "readiness_surfaces.readiness_supervisor.monitor_state")
            self.assertEqual(operator_board["read_order"][8], "readiness_surfaces.readiness_supervisor.current_refs.capture_filtered_out_count")
