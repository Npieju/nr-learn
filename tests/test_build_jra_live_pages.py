...
class BuildJraLivePagesTest(unittest.TestCase):
    ...
    def test_render_root_page_applies_prefix_without_duplicate_segment(self) -> None:
        ...
        self.assertIn('built=2026-06-07 05:30', root_html)
        self.assertIn('2026-06-06 17:15', root_html)
    ...
    def test_render_live_page_has_parent_navigation_and_focused_refresh_control(self) -> None:
        ...
        self.assertIn('function recomputeOddsDependentRaceFields(rows) {', html)
        self.assertIn('assignDescendingRank(rows, "expected_value", "ev_rank", ["score", "odds"]);', html)
        self.assertIn('.harville-overview-value {', html)
        self.assertIn('width: 1%;', html)
        self.assertIn('.harville-overview-table tbody td:first-child {', html)
        self.assertIn('.harville-overview-table thead tr:first-child th:first-child {', html)
        self.assertNotIn('.harville-overview-table th:first-child,', html)
        ...
        self.assertIn('state.data.metadata.policySelectedRows = state.data.races', html)
