...
- [../scripts/run_netkeiba_2026_live_publish.py](../scripts/run_netkeiba_2026_live_publish.py)
  - same-day 公開運用の入口。live rerun と Pages build を束ね、必要なら `pages/` の commit / push まで進める。
...
この 2 本は役割を分けて読む。`run_local_nankan_provenance_audit.py` は `classification_basis=pre_race_feature_availability` を使う trust-bearing 判定であり、historical row が model input として pre-race-valid かを決める。一方で `run_local_nankan_source_timing_audit.py` は `classification_basis=snapshot_time_vs_scheduled_post_time` を使う fetch-timing / recoverability audit であり、historical fetch の運用履歴を読む。したがって source timing が `future_only_pre_race_capture_available` のままでも、historical pre-race benchmark handoff 自体は completed まで進み得る。
...
も束ねるので、`#101/#103` の current phase はまずこの board を一次参照にする。bootstrap handoff が `completed` まで進んだ場合は、同 surface に `evaluation_pointer_output` と `evaluation_pointer_payload` も入り、local formal-read の入口を board からそのまま辿れる。
...
この handoff は source timing の negative read を再判定する場所ではない。`pre_race_summary` 側は `#120` trust split を通った materialized corpus を前提に進み、`benchmark_manifest.status=completed` は train/evaluate を skip した readiness-only run でも出る。historical benchmark を formal rerun するときは、この manifest と benchmark gate manifest から再開すればよい。
