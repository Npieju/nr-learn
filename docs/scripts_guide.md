...
この bridge 情報は downstream の schema / numeric compare / numeric summary / left gap audit / left recovery plan にも引き継ぐ。さらに status board の `phase_summaries` でも `public_snapshot` 行を含めて同じ値を補完するので、mixed alias を保ったまま生成を続けても、どの段で見ても `requested_revision`, `resolved_left_revision`, `resolved_left_source_kind`, `resolved_left_artifact` を読める。
