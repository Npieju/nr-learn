...
`#159` の current race-level probability control compare を rerun するときは、次を使う。

```bash
/workspaces/nr-learn/.venv/bin/python scripts/run_evaluate.py \
  --config configs/model_catboost_fundamental_strength_no_market_calibrated.yaml \
  --data-config configs/data_2025_latest.yaml \
  --feature-config configs/features_jra_fundamental_no_market.yaml \
  --model-artifact-suffix issue159_smoke \
  --artifact-suffix issue159_norm_compare_full_v1 \
...
/workspaces/nr-learn/.venv/bin/python scripts/run_evaluate.py \
  --config configs/model_catboost_fundamental_strength_no_market_softmax_temp085_calibrated.yaml \
  --data-config configs/data_2025_latest.yaml \
  --feature-config configs/features_jra_fundamental_no_market.yaml \
  --model-artifact-suffix issue159_smoke \
  --artifact-suffix issue159_softmax_temp085_compare_full_v1 \
  --start-date 2025-09-01 \
  --end-date 2025-12-31 \
  --max-rows 120000 \
  --wf-mode full
...
- 現在の low-complexity keep-candidate は `configs/model_catboost_fundamental_strength_no_market_softmax_temp085_calibrated.yaml` である。
- baseline summary は `artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_norm_compare_full_v1_20250901_20251231_wf_full_nested.json`。
- keep-candidate summary は `artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_softmax_temp085_compare_full_v1_20250901_20251231_wf_full_nested.json`。
- issue 反映前や shell 1 行で GitHub へ転記するときに backtick を含む本文を扱う場合は、shell 展開事故を避けるため `gh issue comment --body-file <path>` を使う。
...
ただし result-ready 側の historical `local_nankan` alias は、依然として `#121` source timing / recoverability と universe scope の制約が残るため、generic `run_train.py` / `run_evaluate.py` / `run_predict.py` では diagnostic-only read として扱う。diagnostic-only run を意図するときだけ `--allow-diagnostic-local-nankan` を明示する。一方で trust-ready historical corpus は `local_nankan_*_pre_race_ready` profile 群を benchmark reference として使う。
...
- `local_nankan_value_blend_bootstrap_pre_race_ready` は `configs/data_local_nankan_pre_race_ready.yaml` を使う `#103` bootstrap 用 alias である。
- `local_nankan_baseline_pre_race_ready` も `configs/data_local_nankan_pre_race_ready.yaml` を使う。wrapper を使わず generic CLI で strict pre-race corpus を触る baseline / benchmark reference 入口である。
- pre-race-ready 側は live capture の途中状態を含みうるが、current historical corpus では `#101` formal rerun benchmark reference としても使う。主 KPI 判定では revision 固有 artifact を一次参照にする。
...
pre-race-ready dataset 側の benchmark / readiness 確認例:
...
`_2025_latest` suffix は `data_2025_latest.yaml` を付けた派生 profile を広く作るが、2026-04-05 時点で current stable family として日常的に参照するのは次の 4+1 本である。

| profile | 種別 | 代表コマンド | 代表 artifact / window |
| --- | --- | --- | --- |
| `current_best_eval_2025_latest` | evaluation mainline reference | `run_train.py`, `run_evaluate.py`, `run_revision_gate.py`, `run_netkeiba_latest_revision_gate.py` | latest formal evaluation / revision gate / refresh rerun |
...
- `current_best_eval_2025_latest` は `r20260405_2025latest_refresh_reuse_runtimecfg_wfprofilefix` で latest formal refresh が `pass / promote` まで整合し、latest holdout の evaluation reference を formal artifact まで再現できることを確認済みである。
- ただしこの profile は `value_blend` family なので、新 revision suffix の component artifact がまだ無い compare では train をそのまま走らせず、`--skip-train` と `--evaluate-model-artifact-suffix` を付けた reuse-compare を優先する。
- config 自体の output file 名に revision suffix が入っている場合は、artifact path の二重 suffix を避けるため `--evaluate-no-model-artifact-suffix` を使う。latest wrapper でも同 flag をそのまま `run_revision_gate.py` へ渡せる。
...
2025 latest readiness wrapper を value_blend family の artifact 再利用 compare に使う例:
...
- `run_netkeiba_latest_revision_gate.py` でも `--skip-train`、`--train-artifact-suffix`、`--evaluate-model-artifact-suffix` をそのまま `run_revision_gate.py` へ渡せる。
- `run_netkeiba_latest_revision_gate.py` でも `--evaluate-no-model-artifact-suffix` をそのまま `run_revision_gate.py` へ渡せる。
- `current_best_eval_2025_latest` のような value_blend family で new revision suffix の component artifact が未作成なら、まず reuse compare として上の導線を使う。
...
当日オッズ更新 rerun から Pages publish まで一気に進めたいときの wrapper:

```bash
/workspaces/nr-learn/.venv/bin/python scripts/run_netkeiba_2026_live_publish.py \
  --race-date 2026-04-05 \
  --headline-contains 大阪杯 \
  --mode full \
  --odds-refresh
...
```bash
/workspaces/nr-learn/.venv/bin/python scripts/publishing/run_build_jra_live_pages.py \
...
same-day 完了後に enriched benchmark rerun の入口を切りたいときの wrapper:

```bash
/workspaces/nr-learn/.venv/bin/python scripts/run_netkeiba_2026_benchmark_gate.py \
  --skip-train \
  --skip-evaluate
```
...
- cycle 完了は post-cycle snapshot 実行前に backfill manifest へ書き戻されるので、snapshot / status board 側から completed cycle 数が 1 cycle 遅れにならない。
...
- `run_netkeiba_2026_live_publish.py` は same-day 公開向け wrapper で、`mode=full` / `rerun_only` / `publish_only` を持つ。`mode=full` では live handoff を完了させてから Pages build までつなぎ、`mode=publish_only` では既存 prediction artifact から Pages だけを再生成する。
- `run_netkeiba_2026_live_publish.py --odds-refresh` は `--force --refresh-live-crawl` をまとめて有効化する。当日オッズ更新を前提にした rerun ではこの shorthand を既定入口として使う。
- live CLI / handoff / publish wrapper は `--model-artifact-suffix` を受ける。versioned artifact を固定して live 再生したいときはこれを使う。
- `current_recommended_serving_baseline_r20260325_2025_latest` は formal baseline revision `r20260325_current_recommended_serving_2025_latest_benchmark_refresh` を既定 artifact として読む replay 用 profile である。baseline lineage を残したまま current profile と比較したいときの入口に使う。
- `run_netkeiba_2026_live_publish.py` は `artifacts/reports/netkeiba_2026_live_publish_manifest.json` に mode、prediction file、`odds_official_datetime_max`、target page、git status を残す。status board もこの manifest を読んで publish 状態を表示する。
- `scripts/publishing/run_build_jra_live_pages.py` は、`predictions_*_jra_live.csv` と sibling の `.summary.json` / `.live.json` を読み、`pages/jra-live/<race-date>/index.html` と `data.json` を生成する。root の `pages/index.html` も更新し、`.github/workflows/publish-pages.yml` と組み合わせると GitHub Pages へそのまま deploy できる。
- `run_netkeiba_2026_status_board.py` は、`artifacts/reports/netkeiba_backfill_manifest_2026_ytd.json`、`artifacts/reports/netkeiba_coverage_snapshot_2026_ytd.json`、`artifacts/reports/netkeiba_2026_live_handoff_manifest.json`、`artifacts/reports/netkeiba_2026_live_publish_manifest.json`、`artifacts/reports/netkeiba_2026_benchmark_gate.json` を束ね、`status`、`current_phase`、`recommended_action`、`highlights` を 1 回で読めるようにする。top-level `status` は少なくとも `running` / `waiting` / `ready` / `handed_off` / `completed` / `failed` を区別し、進行中は crawl target manifest と実 lock file から `active_cycle`、各 target の `processed_ids`、`rows_written`、live `crawl_lock` を反映する。history frontier は外部 result/racecard CSV の max date を直接読み、`race_result_gap_days`、`race_card_gap_days`、`limiting_history_target` により target date までの残日数も確認できる。handoff 後は live summary/runtime manifest から `policy_selected_rows`、`num_races`、`odds_official_datetime_max`、publish manifest から `page_url` と git 状態を表示し、target page/data file を最後に変更した `pages_commit_sha` も derived provenance として表示する。post-race benchmark gate が存在する場合、top-level `current_phase` と `recommended_action` も benchmark 側を優先する。
...
- `run_netkeiba_2026_benchmark_gate.py` は、2026 YTD same-day 直後の post-race benchmark rerun 用 wrapper で、enriched data/model/feature config と専用 manifest 名を束ねる。既定では完了後に status board も更新する。
...
local_nankan の current blocker を 1 本の board で読む入口:

```bash
/workspaces/nr-learn/.venv/bin/python scripts/run_local_nankan_status_board.py
```

補足:

- output は `artifacts/reports/local_nankan_data_status_board.json`。
- top-level `status`, `current_phase`, `recommended_action` は current blocker を要約する。
- `readiness_surfaces` では次をまとめて読める。
...
- child manifest に降りるときも `read_order` を使える。`local_nankan_readiness_watcher_manifest.json` は `status -> current_phase -> recommended_action -> probe_summary.status -> probe_summary.result_ready_races -> probe_summary.pending_result_races`、`local_nankan_result_ready_bootstrap_handoff_manifest.json` は `status -> current_phase -> recommended_action -> handoff_manifest.status -> handoff_manifest.current_phase -> handoff_manifest.recommended_action` の順で読む。`bootstrap_command_plan` の最後には `run_local_evaluate.py` が含まれるので、result-ready 到達後の follow-up は local evaluation pointer まで同じ wrapper で閉じられる。bootstrap handoff が `completed` のときは、同 manifest と canonical status board の `readiness_surfaces.bootstrap_handoff` に `evaluation_pointer_output` と `evaluation_pointer_payload` も出る。
- さらに `local_nankan_pre_race_readiness_probe_summary.json` は `status -> current_phase -> recommended_action -> materialization_summary.result_ready_races -> materialization_summary.pending_result_races -> historical_source_timing.status`、`local_nankan_pre_race_benchmark_handoff_manifest.json` は `status -> current_phase -> recommended_action -> pre_race_summary.status -> pre_race_summary.current_phase -> benchmark_manifest.status` の順で読める。
- provenance と source timing はここで混ぜない。historical trust 判定は `local_nankan_provenance_audit.json` の `classification_basis=pre_race_feature_availability` を、fetch-timing の recoverability は `local_nankan_source_timing_audit.json` の `classification_basis=snapshot_time_vs_scheduled_post_time` を読む。したがって `historical_source_timing.status=future_only_pre_race_capture_available` でも、pre-race handoff / benchmark gate が completed になり得る。
- lowest-level artifact では `local_nankan_source_timing_audit.json` を `status -> current_phase -> recommended_action -> historical_pre_race_recoverability.result_ready_pre_race_rows -> historical_pre_race_recoverability.future_only_pre_race_rows -> historical_pre_race_recoverability.status`、`local_nankan_pre_race_capture_coverage_summary.json` を `status -> current_phase -> recommended_action -> pre_race_only_rows -> result_ready_races -> pending_result_races` の順で読む。
