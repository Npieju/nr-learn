...
- `run_netkeiba_2026_status_board.py` は、`artifacts/reports/netkeiba_backfill_manifest_2026_ytd.json`、`artifacts/reports/netkeiba_coverage_snapshot_2026_ytd.json`、`artifacts/reports/netkeiba_2026_live_handoff_manifest.json` を束ね、`status`、`current_phase`、`recommended_action`、`highlights` を 1 回で読めるようにする。進行中は snapshot の `target_states` と `crawl_lock` から `active_cycle`、各 target の `processed_ids`、`rows_written` を反映し、history frontier は外部 result/racecard CSV の max date を直接読む。
