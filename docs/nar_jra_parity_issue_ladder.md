...
- strict pre-race benchmark rebuild:
  - `#101` open, but blocked on `#120/#121/#122`
- pre-race capture expansion:
  - `#102` close

したがって current blocker は model family ではなく trust-bearing dataset readiness である。`#102` は negative read で閉じ、current primary path は `#120 -> #121 -> #122 -> #101` の順に narrow されている。
...
3. `#122` future-only readiness handoff
4. `#101` result-ready rerun
5. `#103`
