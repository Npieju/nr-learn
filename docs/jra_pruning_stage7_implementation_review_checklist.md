...
- `scripts/run_pruning_rollout_guardrails_check.py`
...
- [ ] retained anchors が次の 5 columns で固定されている
  ...
  - `障害区分`
  - `sex`
- [ ] `発走時刻`, `東西・外国・地方区分` は candidate config に declaration が残っていても `exclude_columns` 側で effective selected surface から外れている
- [ ] group 単位の説明が可能である
  - calendar context
  - recent-history core
...
- [ ] `python scripts/run_pruning_rollout_guardrails_check.py` が `status=pass`, `decision=review_ready` を返す
...
## 9. Issue Decision Template

reviewer が issue `#124` に残す decision summary は、次のどちらかを最小形として使う。

### Approve Template

```text
Decision:
- approve `stage-7` as an internal implementation-candidate review package

Boundary:
- keep `stage-7` as the defended stopping point
- keep `stage-8 condition quartet` as hold-boundary material
- do not use this as a public benchmark update reason

Why:
- exact diff remains bounded at the documented 45-column removal set
- rollback path is concrete and group-wise
- formal / actual-date / mechanical package checks support review-readiness

Next execution source:
- `docs/issue_library/next_issue_pruning_stage1_calendar_recent_history_bundle.md`
```

### Keep Docs-Only Template

```text
Decision:
- keep `stage-7` as docs-only / analysis reference for now

Boundary:
- do not proceed to staged execution review yet
- do not use this as a public benchmark update reason

Why:
- stage-7 stopping point itself is supported, but the implementation unit is not yet judged tight enough for approval
- retain current rollback / validation material as review and archive support only

Next move:
- return the JRA queue to selecting a different single measurable hypothesis
```