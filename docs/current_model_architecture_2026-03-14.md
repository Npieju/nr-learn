...
- representative 5-date calendar でも同じ結論が崩れないか確認するため、`fallback_hybrid_june_strict` profile も追加して `2024-05-25`, `2024-06-15`, `2024-07-20`, `2024-08-10`, `2024-09-14` を再検証した。旧 fallback との compare では差分は `2024-06-15` の 1 bet だけに縮退し、shared-date aggregate は `policy_bets 14 -> 13`, mean `policy_roi` 同値 `1.875`, total return 同値 `15.0`, total net `1.0 -> 2.0` だった。May 25 は policy name が `may_june_runtime_kelly` から `may_runtime_kelly` へ変わるが realized outcome は同じで、July/August/September も fallback と完全一致した。
- representative best compare でも June-strict probe は quantitative outcome が full 5 dates で完全一致し、差は `2024-05-25` の `score_source` ラベルだけだった。つまり current live 10-date slice だけでなく、現行の representative month calendar 全枝でも「best の observed serving behaviorを default-score single-stack 側へほぼそのまま写せている」とみなしてよい。
- 2026-03-19 の追加で `scripts/run_serving_smoke.py` は preset にない `--date` でも expected `score_source` / `policy_name` を config の serving regime から自動解決できるようになった。これにより、long tail の actual calendar validation を profile 追加なしで回せるようになった。
- その機能で `2024-06-29..2024-09-15` の全 weekend 24 dates を actual tail-window として再検証した。`serving_smoke_best_policy_may_tail_weekends.json` と `serving_smoke_fallback_hybrid_june_strict_tail_weekends.json` は 24 shared dates すべてで `score_source` / `policy_name` / `policy_bets` / `policy_roi` / total return / total net が完全一致し、aggregate は `policy_bets=82`, mean `policy_roi=1.0697`, total return `85.1`, total net `3.1` だった。したがって June-strict rollback は June 以降の actual weekend calendar でも best をそのまま再現している。
- 同じ 24-date tail-window で旧 fallback と比較すると、差分は `2024-06-29` と `2024-06-30` の June 最終 weekend 2 日だけだった。policy 名は `june_runtime_kelly` vs `may_june_runtime_kelly` で、bets は `82 vs 85`、mean `policy_roi` は June-strict/best `1.0697` vs old fallback `1.0718` とわずかに fallback が高い一方、total return は `85.1 vs 85.2491` とほぼ同値、total net は June-strict/best `3.1` vs old fallback `0.2491` で June-strict/best が `+2.8509` units 改善した。
- その内訳は `2024-06-29` と `2024-06-30` の extra June bets に尽きる。`2024-06-29` は June-strict/best `2 bets / ROI 0.0 / net -2.0` に対して old fallback `4 bets / ROI 0.0373 / return 0.1491 / net -3.8509`、`2024-06-30` は `2 bets / ROI 0.0 / net -2.0` に対して `3 bets / ROI 0.0 / net -3.0` だった。つまり old fallback は June 最終 weekend でごく小さな追加 return を拾うが、その対価として net は悪化しており、tail-window でも June stricter threshold を keep する判断は維持される。
