...
2026-04-06 時点で current NAR corpus は `local Nankan`、すなわち South Kanto local track に限られている。したがって current line は `full NAR baseline` ではなく、あくまで `South-Kanto-only readiness track` として読む必要がある。
...
- South-Kanto-only sample の高い ROI を、そのまま NAR 全体や JRA 相当の信頼性へ外挿すると誤読しやすい。
...
- South-Kanto-only line を `NAR 全体の reliable ROI` と読まない
- current high ROI を JRA 相当の trust level と見なさない
- universe coverage と ROI reliability が未解決の間は readiness / audit track として扱う
...
ただしこれは `local Nankan` の data readiness があるという意味であり、NAR universe coverage 問題が解けたという意味ではない。

また、historical diagnostics では次も確認済みである。
...
したがって current local Nankan line の high ROI は未解決の reliability 問題を含んでおり、JRA 相当の trust level をまだ主張しない。

したがって、「NAR は後で考える」段階はもう過ぎている一方で、「local Nankan の高 ROI をそのまま信用する」段階でもない。正しい next step は、JRA の知見を参考にしながら、NAR を South-Kanto-only separate universe readiness track として扱い、universe coverage と ROI reliability を明示的に監査することである。
