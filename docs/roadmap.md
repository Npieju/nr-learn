...
また、旧 JRA market-aware probability-path branch は first candidate・support diagnostics・bounded residual follow-up まで negative read で閉じ、current JRA next action は broader residual continuation ではなく `#159/#160` の race-level probability architecture 比較に戻した。low-complexity control としては market-free CatBoost ranking artifact に explicit score calibration を付与し、race closure を `normalize_score_sum`、softmax `temp=0.7/0.85/0.9` で比較した。[../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_norm_compare_full_v1_20250901_20251231_wf_full_nested.json](../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_norm_compare_full_v1_20250901_20251231_wf_full_nested.json) と [../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_softmax_temp085_compare_full_v1_20250901_20251231_wf_full_nested.json](../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_softmax_temp085_compare_full_v1_20250901_20251231_wf_full_nested.json) では、`temp=0.85` が `wf_nested_test_roi_weighted=0.7848`、`ev_top1_roi=1.0480` で normalize baseline の `0.7541`、`1.0400` を上回り、current low-complexity keep-candidate になった。一方で AUC / logloss は normalize baseline より悪く、確率品質 trade-off は残るため、現時点では promote ではなく architecture control candidate として保持する。

### 4.19 prediction foundation diagnostics の close と broader composition branch への復帰

- [../artifacts/reports/evaluation_summary_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_af0b3a870abf79f4_wf_off_nested.json](../artifacts/reports/evaluation_summary_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_af0b3a870abf79f4_wf_off_nested.json) により、aligned baseline control は `ev_threshold_1_0_bets=5009`, `stability_assessment=representative` を維持した。
- [../artifacts/reports/evaluation_summary_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_ee346cbcc6343bc3_wf_off_nested.json](../artifacts/reports/evaluation_summary_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_ee346cbcc6343bc3_wf_off_nested.json) により、aligned sidecar control も `ev_threshold_1_0_bets=5014`, `stability_assessment=representative` を維持した。
- したがって `123 features + alpha component` や `full_training_table + wf_off` geometry は support collapse の主因ではない。残る主要差分は `market_aware_alpha_branch` の composition path である。
- このため [issue_library/next_issue_jra_prediction_foundation_probability_support_diagnostics.md](issue_library/next_issue_jra_prediction_foundation_probability_support_diagnostics.md) は `reject` で閉じ、JRA next branch は prediction foundation continuation ではなく [issue_library/next_issue_jra_broader_composition_support_preserving_probability_path.md](issue_library/next_issue_jra_broader_composition_support_preserving_probability_path.md) に戻す。
- 2026-04-19 に broader composition の first candidate `support_preserving_residual_path` を実装し、[../artifacts/reports/evaluation_summary_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_da9a84b67bd0d1c4_wf_off_nested.json](../artifacts/reports/evaluation_summary_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_da9a84b67bd0d1c4_wf_off_nested.json) で `ev_threshold_1_0_bets=5128`, `stability_assessment=representative` を確認した。representative support は preserved である。
- 同日の September difficult-window compare では [../artifacts/reports/serving_smoke_compare_sep25_support_preserving_prob_v1_base_vs_cand.json](../artifacts/reports/serving_smoke_compare_sep25_support_preserving_prob_v1_base_vs_cand.json) と [../artifacts/reports/serving_smoke_compare_sep25_support_preserving_prob_v1_sidecar_vs_cand.json](../artifacts/reports/serving_smoke_compare_sep25_support_preserving_prob_v1_sidecar_vs_cand.json) により、candidate は `37 bets / total net -11.9` で `8/8 zero-bet` を回避した。baseline `34 bets / -13.6` より net は改善したが、sidecar `35 bets / -9.9` を超える read はまだ弱い。
- したがって broader composition issue は `hold` とし、support-preserving compare surface を成立させたところで閉じる。next step は serving default 更新ではなく、human review 後に別 issue で bounded residual follow-up を切るか判断する。
- その後の bounded residual calibration follow-up では `market_residual_positive_only: true` と `market_residual_weight: 0.05` の 1 candidate を追加したが、[../artifacts/reports/evaluation_summary_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_04dd633acf0ad297_wf_off_nested.json](../artifacts/reports/evaluation_summary_catboost_value_stack_lgbm_roi_high_coverage_tune_roi012_liquidity_regime_hybrid_04dd633acf0ad297_wf_off_nested.json) の representative read は first candidate と同一だった。September difficult-window compare でも [../artifacts/reports/serving_smoke_compare_sep25_support_preserving_prob_positive_only_v1_base_vs_cand.json](../artifacts/reports/serving_smoke_compare_sep25_support_preserving_prob_positive_only_v1_base_vs_cand.json) と [../artifacts/reports/serving_smoke_compare_sep25_support_preserving_prob_positive_only_v1_sidecar_vs_cand.json](../artifacts/reports/serving_smoke_compare_sep25_support_preserving_prob_positive_only_v1_sidecar_vs_cand.json) が `37 bets / total net -11.9` を返し、first candidate 比で有意差を作れなかった。
- したがって `positive_only + lower_weight` calibration follow-up は `reject` とし、support-preserving residual family をこれ以上 ad-hoc に掘るのではなく、JRA broader composition の次手は human review と issue recut を優先する。

### 4.20 JRA race-level probability architecture の softmax control 比較

- `#159/#160` の current control として、market-free CatBoost ranking artifact に explicit calibration を付けた race-level probability closure を比較した。
- `softmax temp=1.0` は live prediction probe でも `normalize_score_sum` と実質同値で no-op だった。
- `wf_full_nested` の 2025-09-01..2025-12-31 compare では、normalize baseline [../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_norm_compare_full_v1_20250901_20251231_wf_full_nested.json](../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_norm_compare_full_v1_20250901_20251231_wf_full_nested.json) が `wf_nested_test_roi_weighted=0.7541048868`, `ev_top1_roi=1.04`, `auc=0.7529930120`, `logloss=0.3416275856` だった。
- softmax `temp=0.7` は [../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_softmax_temp07_compare_full_v1_20250901_20251231_wf_full_nested.json](../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_softmax_temp07_compare_full_v1_20250901_20251231_wf_full_nested.json) で `wf_nested_test_roi_weighted=0.7781411707` まで改善したが、`ev_top1_roi=1.0395370370`, `auc=0.7461460287`, `logloss=0.3585593566` と probability quality 側は悪化した。
- softmax `temp=0.9` は fast compare で `wf_nested_test_roi_weighted=0.6647229702` と baseline を下回り、戻し過ぎと判断した。
- softmax `temp=0.85` は [../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_softmax_temp085_compare_full_v1_20250901_20251231_wf_full_nested.json](../artifacts/reports/evaluation_summary_catboost_fundamental_strength_no_market_model_issue159_softmax_temp085_compare_full_v1_20250901_20251231_wf_full_nested.json) と [../artifacts/reports/evaluation_wf_progress_catboost_fundamental_strength_no_market_model_issue159_softmax_temp085_compare_full_v1_20250901_20251231_wf_full_nested.json](../artifacts/reports/evaluation_wf_progress_catboost_fundamental_strength_no_market_model_issue159_softmax_temp085_compare_full_v1_20250901_20251231_wf_full_nested.json) で `wf_nested_test_roi_weighted=0.7847891598`, `ev_top1_roi=1.0479629630` を示し、current low-complexity keep-candidate になった。
- ただし `temp=0.85` でも `auc=0.7464666459`, `logloss=0.3579295431` と normalize baseline より悪い。したがって current conclusion は「long-horizon ROI priority では keep-candidate、probability quality では unresolved trade-off」であり、promote ではなく architecture comparison の control として保持する。
