...
対外向けに現状の数値を説明したい場合は [public_benchmark_snapshot.md](public_benchmark_snapshot.md) を、benchmark 数字と operational role の関係まで説明したい場合は [public_benchmark_operational_reading_guide.md](public_benchmark_operational_reading_guide.md) を参照する。
...
local-only universe の benchmark を読むときは、順序を別にする。

1. `local_public_snapshot_<revision>.json` で public 向けの status / readiness / decision を確認する。`backfill_handoff_summary` があれば、Phase 0 の handoff 停止点もここで先に読む。
...
`missing_left_rows` の中身を具体的に潰したいときは `mixed_universe_left_gap_audit_<left_universe>_vs_<right_universe>_<revision>.json` に進む。ここでは各 row に必要な left artifact と command preview が出るので、次にどの local lineage step を実行すべきかを機械的に読める。upstream preflight blocker だけでなく local backfill/materialize handoff blocker があるときも、その `recommended_action` を同時に読める。

row ごとの情報を実行 plan にまとめたいときは `mixed_universe_left_recovery_plan_<left_universe>_vs_<right_universe>_<revision>.json` を使う。ここでは `run_revision_gate` や `run_local_evaluate` のような command を重複除去して、どの missing row 群を埋めるかを一段薄く読める。command がまだ無い段階でも、preflight blocker や local handoff blocker を手動 step として保持し、upstream blocker がある場合はそれが first step になる。
...
`current_tighter_policy_search_candidate_2025_latest` は formal support 側では有望だったが、operational role は actual-date の fresh compare で分けて読む必要がある。

| window | baseline | tighter policy candidate | 読み方 |
| --- | --- | --- | --- |
| `2025-09` 8 日 | `32 bets / -27.3 / 0.2959` | `9 bets / -4.3 / 0.8395` | September difficult window では強い損失圧縮 |
| `2025-12` tail 8 日 | `45 bets / +21.8 / 1.6712` | `9 bets / +21.4 / 1.6032` | 利益局面では baseline が依然優位 |

September fresh compare では、tighter policy candidate は score source を変えずに bet 数を `32 -> 9` まで絞り、total net を `-27.3 -> -4.3`、pure bankroll を `0.2959 -> 0.8395` まで改善した。一方で December tail の fresh compare では、bet 数を `45 -> 9` まで削っても total net は `+21.8 -> +21.4` とほぼ同水準に留まり、pure bankroll は `1.6712 -> 1.6032` で baseline を下回った。したがって現時点の読み方は「broad baseline replacement」ではなく、「September difficult regime で exposure を切る analysis-first defensive candidate」である。

formal support 境界の threshold sweep もすでに確認済みである。`wf_threshold_sweep_current_tighter_policy_search_candidate_2025_latest.json` では、現行の `min_bet_ratio=0.03 / min_bets_abs=100` で `4/5 feasible folds`、`0.03 / 80` で `5/5 feasible folds` になった。新たに通るのは fold 2 だけで、best feasible は既存 family と同じ `kelly blend_weight=0.8 / min_prob=0.03 / odds_max=25`、`98 bets / ROI 0.7542 / final_bankroll 0.9199 / max_drawdown 0.1098` だった。さらにこの point は `r20260327_tighter_policy_ratio003_abs80` として threshold-only revision gate でも `pass / promote` し、formal benchmark は `weighted ROI 1.1042 / feasible folds 5/5 / bets 598` だった。つまり `0.03/80` は別系統の aggressive candidate を入れる変更ではなく、既存の defensive family を formal support 上でどこまで許容するかの境界調整として読むべきである。ただし serving policy 自体は不変なので、September で defensive、December で baseline 優位という actual-date role はこの threshold 変更だけでは変わらない。

### 5.4 recent-heavy true retrain snapshot

date-based train window を短くした recent-heavy 比較では、2018 start / 2020 start の true retrain run がともに formal に通過している。

| revision | 対象 | decision | 主要値 |
| --- | --- | --- | --- |
| `r20260327_recent_2018_component_retrain` | `current_recommended_serving_2025_recent_2018` family | `pass / promote` | AUC `0.8432`, top1 ROI `0.8166`, EV top1 ROI `0.7400`, nested WF weighted test ROI `0.9595`, feasible folds `5/5`, bets `767` |
| `r20260327_recent_2020_component_retrain` | `current_recommended_serving_2025_recent_2020` family | `pass / promote` | AUC `0.8449`, top1 ROI `0.8186`, EV top1 ROI `0.7496`, nested WF weighted test ROI `0.9218`, feasible folds `4/5`, bets `878` |
...
この seasonal reading を latest 2025 actual-date compare に持ち込むときも、判断ルールは同じである。September difficult window の改善だけで broad replacement と結論せず、December tail control と pair で読んだうえで `operational seasonal alias` か `analysis-first defensive candidate` かを切り分ける。以後の標準文面は `docs/seasonal_derisk_decision_standard.md` を正本にする。
