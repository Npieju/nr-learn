...
- late-September では pure path の final bankroll が baseline `0.2780`、candidate `1.0000` で、best path も pure candidate 側だった
...
2. current evidence の範囲では、May/August/tail を壊さず late-September だけ改善できている
3. 次に進めるなら、この candidate を aggregate / dashboard / wider-window compare の中で再確認するのが自然である
