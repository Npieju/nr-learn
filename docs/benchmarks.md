...
formal support 境界の threshold sweep もすでに確認済みである。`wf_threshold_sweep_current_tighter_policy_search_candidate_2025_latest.json` では、現行の `min_bet_ratio=0.03 / min_bets_abs=100` で `4/5 feasible folds`、`0.03 / 80` で `5/5 feasible folds` になった。新たに通るのは fold 2 だけで、best feasible は既存 family と同じ `kelly blend_weight=0.8 / min_prob=0.03 / odds_max=25`、`98 bets / ROI 0.7542 / final_bankroll 0.9199 / max_drawdown 0.1098` だった。つまり `0.03/80` は別系統の aggressive candidate を入れる変更ではなく、既存の defensive family を formal support 上でどこまで許容するかの境界調整として読むべきである。ただし serving policy 自体は不変なので、September で defensive、December で baseline 優位という actual-date role はこの threshold 変更だけでは変わらない。
