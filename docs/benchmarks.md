...
| recent-heavy retrain candidate | `current_recommended_serving_2025_recent_2020` family | `pass / promote` | true retrain 比較用の analysis-first 候補 |
...
### 5.3 recent-heavy true retrain snapshot

date-based train window を短くした recent-heavy 比較では、2020 start の true retrain run が次まで完了している。
...
この run は recent-heavy split の学習効果を本当に見るため、win / roi component を同一 suffix で再学習してから stack を再 bundle している。

解釈としては、AUC / logloss / EV 系は改善した一方で、nested WF weighted test ROI と bets は直前の pseudo-retrain run より低下した。したがって現時点では、recent-heavy true retrain は「formal に通過した比較候補」であって、baseline 置換の確定根拠ではない。
