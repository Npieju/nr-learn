...
したがって、support の読みは前段より一段強い。`deepest_stage_selected_present` は complete bad-day separator ではないが、少なくとも今ある cross-window evidence では `positive` day に一度も出ておらず、high-risk subset flag としての precision は高い。次に広げるべきのは stage2/3 閾値の調整ではなく、この `deepest-stage selected` signal の support をもっと長い window で増やすことである。
