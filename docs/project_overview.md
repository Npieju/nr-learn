...
ただし、これらの run は baseline の代替として即採用するのではなく、recent-heavy learning window の効果を測る analysis-first candidate として扱う。現時点では 2018 start が formal support で優位だが、latest baseline を置き換えるには actual-date compare が必要である。
...
1. recent-heavy true retrain の上位候補である `2018` start が、actual-date compare でも latest baseline より有利か。
