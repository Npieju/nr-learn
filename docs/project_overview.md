...
2026-04-05 時点で、現在の位置づけは次の 5 本に整理している。

| 区分 | profile | 位置づけ | 状態 |
| --- | --- | --- | --- |
| operational baseline | `current_recommended_serving_2025_latest` | 現在の既定運用 profile | formal gate `pass / promote` |
| evaluation mainline reference | `current_best_eval_2025_latest` | latest holdout 上の評価基準線 | latest formal refresh `pass / promote` |
...
ここで重要なのは、`pass / promote` した候補が増えても、だからといって自動で baseline を差し替えていないことである。`nr-learn` では formal 通過と operational 採用を分けて扱う。`current_best_eval_2025_latest` も latest formal refresh が再現できる状態にあるが、score source が 2 本の evaluation reference として保持し、既定運用 profile には置かない。また recent-heavy family では、profile 名だけの train ではなく、component を再学習した true retrain run だけを比較根拠に採る。

2026-04-05 時点の実務上の読み方は単純である。既定運用は baseline を維持し、September difficult regime だけを defensive candidate 群の比較対象にする。December tail のような control window では、候補が formal に通っていても baseline 優位を崩さない。formal 側では `current_best_eval_2025_latest` の latest wrapper rerun も再現可能になったため、evaluation 参照線と operational baseline の両方を source-of-truth から追える。
...
runtime 面では、`configs/data_2025_latest.yaml` に freshness-guarded primary tail cache が default で入り、current baseline smoke は faster path 上でも same-summary equivalent を確認済みである。さらに latest revision gate wrapper も `current_best_eval_2025_latest` の reuse-compare formal refresh まで end-to-end で通る状態になった。したがって直近の queue は runtime 障害対応ではなく docs drift 点検と future option の切り分けへ戻してよい。
...
したがって current queue は、JRA latest の source-of-truth docs を保守しつつ、future option は JRA baseline と切り分けて扱う、という整理で固定する。NAR separate-universe baseline formalization は future option の主候補として保持するが、current baseline 更新の直列タスクには混ぜない。
...
| `current_best_eval_2025_latest` | `r20260405_2025latest_refresh_reuse_runtimecfg_wfprofilefix` | `pass / promote` | formal benchmark `0.8372`, nested WF `0.9157` | `2364` |
...
明示的な docs 導線整理は一巡完了しているが、いま残っているのは定期点検だけではない。

1. current reading が今後の更新でも docs 間で発散しないかを定期的に点検すること。
2. JRA mainline の compare surface を snapshot baseline として固定し、prediction foundation / market deviation / execution policy の 3 track に分けて次 issue を切ること。
3. JRA `market_deviation` の current best candidate `r20260418_jra_lightgbm_alpha_race_norm_v1` を formal signal-source reference として扱い、mainline benchmark judgement へ直接混ぜないこと。
4. JRA 本線の次 experiment は毎回 roadmap stage にぶら下げ、Stage 4 では bounded policy reintegration issue 1 本に固定して進めること。

地方競馬データ拡張については、直近の本線 issue とは切り離して扱うが、棚上げではない。現時点の整理は、JRA latest baseline を詰める仕事とは切り離し、`separate universe` として formalize する、である。NAR は JRA と無理に統合せず、同じ development standard / gating / artifact discipline を移植した別 line として進める。正本は [nar_model_transfer_strategy.md](nar_model_transfer_strategy.md) とする。

その separate-universe 読みでは、historical line の trust semantics はすでに修正済みである。`#120` provenance trust gate は current alias で `pre_race=729107`, `post_race=0`, `unknown=0`, `strict_trust_ready=true` を返し、`#121` source timing audit は fetch-timing / recoverability を別軸として保持する。これにより `#101` formal rerun `r20260415_local_nankan_pre_race_ready_formal_v1` が trust-ready historical corpus 上で完走し、NAR current benchmark reference が確立した。一方で local Nankan only という universe coverage gap と operator-default future-only readiness track は残るため、`local_nankan_recommended` を `configs/model_local_baseline_wf_runtime_narrow.yaml` と promoted artifact `r20260330_local_nankan_baseline_wf_runtime_narrow_v1` に固定した current alias は、引き続き operator convenience 用の diagnostic / readiness reference にとどめる。current benchmark reference と current next candidate は `#101` formal rerun artifact と `#103` architecture bootstrap である。

進行中の優先順位と最新の実行順は、書き捨ての会話ログではなく [roadmap.md](roadmap.md) を正本として更新する。

特に JRA の「市場をどう取り込むか」という論点では、単発の tweak を積み上げる前に stage roadmap を更新し、その stage の success / stop condition で run を閉じる。
...
- 対外向け説明だけが必要な場合は [public_benchmark_snapshot.md](public_benchmark_snapshot.md) と [public_benchmark_operational_reading_guide.md](public_benchmark_operational_reading_guide.md) を読む。
