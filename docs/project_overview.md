...
その separate-universe 読みの historical line には、なお strict trust blocker がある。`#120` provenance trust gate と `#121` source timing audit により、historical local Nankan result-ready rows の大半は `provable pre-race market capture` を持たず、repaired strict audit でも `pre_race=0`, `post_race=728850`, `unknown=257` が確認されている。したがって、`local_nankan_recommended` を `configs/model_local_baseline_wf_runtime_narrow.yaml` と promoted artifact `r20260330_local_nankan_baseline_wf_runtime_narrow_v1` に固定した current alias は、operator convenience 用の diagnostic / readiness reference にとどめる。`artifacts/reports/evaluation_summary_local_nankan_baseline_model_r20260412_local_nankan_recommended_wf_runtime_narrow_full.json` にある `top1_roi=0.8382`, `ev_top1_roi=1.9408`, `auc=0.8775` は、historical local Nankan の trust-carrying benchmark evidence や JRA benchmark 主判定を置き換えるものではない。
