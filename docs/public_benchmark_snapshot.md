...
### 3.4 recent-heavy retrain の初回結果

2026-03-27 には、train window を `2020-01-01..2024-12-31` に絞った recent-heavy true retrain も formal に通過した。

| 指標 | 値 |
| --- | ---: |
| revision | `r20260327_recent_2020_component_retrain` |
| decision | `pass / promote` |
| stability assessment | `representative` |
| AUC | `0.8449` |
| top1 ROI | `0.8186` |
| EV top1 ROI | `0.7496` |
| nested WF weighted test ROI | `0.9218` |
| nested WF bets total | `878` |
| formal benchmark feasible folds | `4 / 5` |

この結果は、recent regime を強く見た学習でも formal support を維持できることを示す。ただし現時点では、運用上の baseline を差し替えるのではなく、learning window 改善の有力候補として保持している。
...
- recent-heavy true retrain でも formal 通過は確認できており、学習 window を直近寄せしても benchmark を維持できる見込みが出ている。
