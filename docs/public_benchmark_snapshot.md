...
この結果により、recent-heavy candidate は formal gate だけでなく actual-date difficult window でも有力候補になった。ただし現時点では operational baseline の切り替えではなく、追加 compare を前提にした候補扱いを維持している。
...
- actual-date compare でも、recent-2018 true retrain は September difficult window で baseline より損失を大きく抑えた。
