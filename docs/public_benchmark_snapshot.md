...
### 3.3 直近の formal pass と改善 candidate

2026-04-05 には、`current_best_eval_2025_latest` の latest formal refresh も完了した。

| 指標 | 値 |
| --- | ---: |
| revision | `r20260405_2025latest_refresh_reuse_runtimecfg_wfprofilefix` |
| profile | `current_best_eval_2025_latest` |
| decision | `pass / promote` |
| stability assessment | `representative` |
| AUC | `0.8364` |
| top1 ROI | `0.7994` |
| nested WF weighted test ROI | `0.9157` |
| formal benchmark weighted ROI | `0.8372` |
| formal benchmark feasible folds | `5 / 5` |

この run は latest holdout の evaluation 参照線を formal artifact まで再現できることを示すもので、現在の operational baseline を置き換えるものではない。

2026-03-26 には、2025 regime 向けに policy search を引き締めた candidate も formal に通過した。
...
この結果は、recent regime を強く見た学習でも formal support を維持できることを示す。ただし、どちらも運用上の baseline を直ちに差し替えるのではなく、learning window 改善候補として保持している。
...
### 4.3 recent-heavy retrain family も September では de-risk を示した

recent-heavy true retrain family でも、`2018-01-01..2024-12-31` window を使った representative compare は 2025-09 の fresh compare で strong de-risk を示した。

| 指標 | Latest baseline | Recent-heavy representative compare |
| --- | ---: | ---: |
| policy bets | `32` | `4` |
| total policy net | `-27.3` | `-4.0` |
| pure path final bankroll | `0.2959` | `0.8557` |

同じ September fresh compare を recent-2020 true retrain にも広げると、`8 bets / -8.0 / 0.7408` だった。したがって recent-heavy family は September difficult window 向けの regime-specific candidate 群として読むのが妥当である。

対外向け snapshot では、recent-heavy family の内部順位づけまでは持ち込まない。必要な場合だけ internal benchmark docs で family 内比較を説明する。

ただし 2025-12 tail の representative fresh compare では、baseline `45 bets / +21.8 / 1.6712` に対して recent-2018 true retrain は `1 bet / -1.0 / 0.9722` だった。つまり recent-heavy candidate は broad replacement ではなく、September difficult window 向けの regime-specific candidate として扱うのが妥当である。
...
- 現在の evaluation mainline reference: `current_best_eval_2025_latest`
...
- 現在の recent-heavy defensive candidate family: `current_recommended_serving_2025_recent_2018` / `current_recommended_serving_2025_recent_2020` true retrain

September difficult regime と December tail control window の読み分けは次のとおりである。

| regime | 既定の読み | defensive option |
| --- | --- | --- |
| September difficult window | baseline を基準にしつつ de-risk variant の差分を参照する | `current_long_horizon_serving_2025_latest`、`current_tighter_policy_search_candidate_2025_latest`、recent-heavy true retrain family |
...
- `current_tighter_policy_search_candidate_2025_latest` と recent-heavy true retrain family は formal に通過しているが、どちらも broad replacement ではない。
...
- latest 2025 holdout の evaluation mainline も formal refresh を再現できる状態にある。
- 直近では support 改善型の candidate も formal に通過し、latest regime に対する改善余地が確認できた。
- recent-heavy true retrain family でも formal 通過は確認できている。
- actual-date compare でも、recent-heavy true retrain family は September difficult window で baseline より損失を大きく抑えた。
...
- September difficult window の public reading では、まず seasonal de-risk alias を見て、その後に tighter policy candidate と recent-heavy true retrain family の defensive read を参照する。
...
- latest eval formal refresh
	- `artifacts/reports/promotion_gate_r20260405_2025latest_refresh_reuse_runtimecfg_wfprofilefix.json`
...
- recent-heavy formal results
	- `artifacts/reports/promotion_gate_r20260327_recent_2018_component_retrain.json`
	- `artifacts/reports/promotion_gate_r20260327_recent_2020_component_retrain.json`
- September / December control compare
	...
	- `artifacts/reports/dashboard/serving_compare_dashboard_current_recommended_serving_2025_latest_sep_full_month_2025_latest_vs_recent2020_true_retrain_fresh_vs_current_recommended_serving_2025_recent_2020_sep_full_month_2025_latest_vs_recent2020_true_retrain_fresh.json`
