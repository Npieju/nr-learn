...
一方で同じ family を December tail の control window に広げると、baseline `45 bets / +21.8 / 1.6712` に対して recent-2018 true retrain は `1 bet / -1.0 / 0.9722` まで縮退した。したがって serving validation の読み方としては、recent-heavy true retrain も tighter policy candidate も broad replacement 候補ではなく、September difficult regime の defensive compare 用 candidate として扱うのが正しい。
