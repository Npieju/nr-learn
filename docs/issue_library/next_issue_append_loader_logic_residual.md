...
fourth accepted cut として、append table にも exact header-list `usecols` を適用した。従来は `join_on=[]` の append table だけ callable `usecols` のままだったが、explicit `keep_columns` / `required_columns` があるなら header から exact list を引けるので、ここを exact-safe に有効化した。

table-level read では次の改善が見えた。

- append source columns: `38 -> 34`
- append load: `~0.62s -> ~0.57s`

reduced smoke も current mainline compare で通っている。
...
したがって current best read は、「append logic residual は `prelimit`、`recent-filter no-copy`、`append keep_columns`、`append exact usecols` の 4 本が accepted であり、current mainline は same-summary のまま `0m22s` 帯まで前進した」である。
