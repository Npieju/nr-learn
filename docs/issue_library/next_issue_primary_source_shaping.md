...
fourth read として、primary tail cache に freshness guard を追加した。
...
- loader behavior:
  - manifest の `tail_rows` が requested rows と一致しない場合は cache 不使用
  - source path / size / mtime が current raw dataset と一致しない場合も cache 不使用
  - その場合は `_read_csv_tail(...)` に自動 fallback する

regression も追加した。

- `tests.test_dataset_loader`:
  - stale cache 作成後に raw source を 1 row 増やし、`load_training_table_tail(...)` が cache を使わず `total_rows=7` と最新 tail を返すことを確認

したがって current best read は、「primary tail cache は stale cache を避ける freshness guard 付きの opt-in candidate として成立した」である。`#36` の残タスクは cache refresh をどう運用するか、つまり default 昇格前に refresh command / freshness policy をどこまで標準化するかに狭まった。
