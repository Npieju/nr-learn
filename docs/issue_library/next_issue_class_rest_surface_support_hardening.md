...
最初の support-hardening candidate `surface interaction only` (`#43`) は formal まで完了し、次の読みで `hold` だった。

- nested summary shape:
  - fold 1 `no_bet`
  - folds 2-5 `portfolio`
- nested summary metrics:
  - `auc=0.8415671649755383`
  - `ev_top1_roi=0.6940128706044587`
  - `wf_nested_test_roi_weighted=0.7462235649546828`
  - `wf_nested_test_bets_total=662`
- promotion gate:
  - `decision=hold`
  - blocking reason: `Walk-forward feasible fold count is below threshold: 2 < 3`
- matching WF feasibility:
  - fold 1 feasible
  - folds 2-4 infeasible
  - fold 5 feasible
  - net feasible folds `2/5`

したがって current next candidate は、surface-only と full 6-feature variant の中間に置く。

## Current Narrower Candidate

次の support-hardening candidate は `surface + class long layoff` とする。
