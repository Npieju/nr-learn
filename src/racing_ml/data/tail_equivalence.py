...
def read_csv_tail_compact_reset_late_candidate(csv_path: Path, tail_rows: int) -> tuple[pd.DataFrame, int]:
    if tail_rows <= 0:
        raise ValueError("tail_rows must be greater than 0")
...
def read_csv_tail_single_chunk_fastpath_candidate(csv_path: Path, tail_rows: int) -> tuple[pd.DataFrame, int]:
    if tail_rows <= 0:
        raise ValueError("tail_rows must be greater than 0")
...
TAIL_READER_CANDIDATES: dict[str, TailReader] = {
    ...
    "single_chunk_fastpath": read_csv_tail_single_chunk_fastpath_candidate,
}
